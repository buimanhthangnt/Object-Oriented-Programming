...
public class Piece {
    ...
    public Piece(TPoint[] points) {
        int[][] mark = new int[12][12];
        body = new TPoint[points.length];
        width = 0;
        height = 0;
        for (int i = 0; i < body.length; i++) {
            if (mark[points[i].x][points[i].y] == 1) continue;
            TPoint newPoint = new TPoint(points[i].x, points[i].y);
            body[i] = newPoint;
    ...
    public Piece computeNextRotation() {
        TPoint[] newArr = new TPoint[body.length];
        for (int i = 0; i < body.length; i++) {
            newArr[i] = new TPoint(this.height - body[i].y - 1, body[i].x);
        }
        return new Piece(newArr);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (this.width != other.width || this.height != other.height ||
                this.body.length != other.body.length) return false;

        int[][] mark = new int[width][height];
        boolean isIdentical = true;
        for (int i = 0; i < body.length; i++) {
            mark[body[i].x][body[i].y] = 1;
        }
        for (int i = 0; i < other.body.length; i++) {
            if (mark[other.body[i].x][other.body[i].y] != 1) isIdentical = false;
        }
        return isIdentical;
    }
	...
	*/
    private static Piece makeFastRotations(Piece root) {
        Piece newPiece1 = root;
        Piece newPiece2 = root;
        do {
            newPiece1.next = newPiece2;
            newPiece1 = newPiece2;
            newPiece2 = newPiece1.computeNextRotation();
        } while (!root.equals(newPiece2));
        newPiece1.next = root;
        return root;
    }
